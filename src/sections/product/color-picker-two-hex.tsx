...
// Converts RGBA to an 8-character HEX string (RRGGBBAA)
function rgbaToHex(r: number, g: number, b: number, a: number): string {
  const rowHex = [r, g, b].map(x => x.toString(16).padStart(2, '0')).join('');
  const alphaHex = Math.round(a * 255).toString(16).padStart(2, '0');
  return (rowHex + alphaHex).toUpperCase();
}

// Parses 3, 6, or 8-character HEX strings back to RGBA
function hexToRgba(hex: string) {
  ...
  if (cleanHex.length === 8) {
    ...
    const g = parseInt(cleanHex.slice(2, 4), 16);
    const b = parseInt(cleanHex.slice(4, 6), 16);
    const a = Math.round((parseInt(cleanHex.slice(6, 8), 16) / 255) * 100) / 100;
    return { r, g, b, a };
  }
  return null;
}

// --- Styled Slider Track Component ---
...
export default function CustomColorPicker({ onChange }: ColorPickerProps) {
  ...
  // Derive active RGB, full 8-digit HEX, and RGBA string output directly
  const { r, g, b } = hsvToRgb(color.h, color.s, color.v);
  const derivedHex8 = rgbaToHex(r, g, b, color.a);
  const rgbaString = `rgba(${r}, ${g}, ${b}, ${color.a})`;

  const [localHex, setLocalHex] = useState(derivedHex8);

  // Send updates upstream to parent layout components
  useEffect(() => {
    if (onChange) onChange(rgbaString);
  }, [rgbaString, onChange]);

  // Keeps the input field in perfect sync with slider changes
  useEffect(() => {
    setLocalHex(derivedHex8);
  }, [derivedHex8]);

  // Handles updating coordinates inside the 2D plane
  const updateSaturationValue = useCallback((clientX: number, clientY: number) => {
  ...
  }, [isDragging, updateSaturationValue]);

  // Handle typing inside the HEX field (Accepts up to 8 chars total)
  ...
  return (
    ...
    >
      ...
      {/* 4. HEX Input Section (Shows full RRGGBBAA code) */}
      <Box sx={{ p: 2, display: "flex", justifyContent: "center", borderTop: "1px solid #f0f0f0" }}>
        <TextField
          ...
          sx={{ width: "150px" }} // Widened slightly to elegantly support 8 characters
        />
      </Box>
    </Paper>
  );
